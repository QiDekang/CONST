...
#  孪生网络融合网络
def create_fusion_layers_sub_1(fusion_feature_size, windows_len):
    ...
    #hiddenLayer_all = Dropout(rate=dropout_rate)(hiddenLayer_all)
    hiddenLayer_all = Dense(size_four, activation='relu', kernel_regularizer=regularizers.l2(l2_rate))(input_features)
    #hiddenLayer_all = Dropout(rate=dropout_rate)(hiddenLayer_all)
    '''
    hiddenLayer_all = Dense(size_four, activation='relu')(hiddenLayer_all)
    hiddenLayer_all = Dropout(rate=dropout_rate)(hiddenLayer_all)
    hiddenLayer_all = Dense(size_five, activation='relu')(hiddenLayer_all)
    hiddenLayer_all = Dropout(rate=dropout_rate)(hiddenLayer_all)
    hiddenLayer_all = Dense(size_six, activation='relu')(hiddenLayer_all)
    hiddenLayer_all = Dropout(rate=dropout_rate)(hiddenLayer_all)
    one_time_output = Dense(1, name='one_time_output')(hiddenLayer_all)
    ## (batch_size, windows_len, 1) -- > (batch_size, windows_len)
    one_time_output = Reshape(target_shape=(windows_len,))(one_time_output)
    multi_time_output = Dense(1, name='multi_time_output')(one_time_output)
    '''
