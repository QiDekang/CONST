...
#  孪生网络融合网络
def create_LSTM_fusion_layers_sub_2(input_features_size, windows_len):

    #  input
    #LSTM_input_features = Input(shape=(windows_len, input_features_size), dtype='float', name='input_features_sub_2')
    #LSTM_input_features = Input(shape=(windows_len, input_features_size,), dtype='float', name='input_features_sub_2')
    LSTM_input_features = Input(shape=(windows_len, input_features_size,), dtype='float')

    #  layers
    hiddenLayer = LSTM(32, input_shape=(windows_len, input_features_size), name='LSTM_2', kernel_regularizer=regularizers.l2(l2_rate))(LSTM_input_features)
    #hiddenLayer = LSTM(32, input_shape=(windows_len, input_features_size), name='LSTM_fusion_layers_sub_2')(LSTM_input_features)
    ...
    #LSTM_output = Dense(1, name='LSTM_fusion_layers_output', kernel_regularizer=regularizers.l2(l2_rate))(hiddenLayer)
