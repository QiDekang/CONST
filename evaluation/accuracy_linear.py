...
#  evaluate、predict
def model_accuracy_linear(save_folder, model_type, predict_model, label_next_ss, label_diff_ss, multi_time_test_label, multi_time_test_t_0_data, test_data, model_std_cols):
    ...
    # predict
    indoor_temp_next_predict_diff, indoor_temp_next_predict_truth = model_predict_linear(save_folder, model_type, predict_model, label_next_ss, label_diff_ss, multi_time_test_label, multi_time_test_t_0_data, test_data, model_std_cols)
    #  save score
...
def model_predict_linear(save_folder, model_type, predict_model, label_next_ss, label_diff_ss, multi_time_test_label, multi_time_test_t_0_data, test_data, model_std_cols):
    ...
    ## baselines
        ...
        x_test = test_data[model_std_cols]
        predict_next_std = predict_model.predict(x_test)
    ...
    if model_type in (['Linear', 'DNN_multi_time', 'LSTM', 'ResNet']):
        predict_truth = label_next_ss.inverse_transform(predict_next_std.reshape(-1, 1))  #  反标准化
        #print('predict_truth\n', predict_truth)
